...
const Negocio = () => {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm({ defaultValues: emp, mode: "onBlur" });
  const {
    control: control2,
  ...
  const _actualizarLogo = async (name) => {
    let res = await API.getBody("emp/update/logo", "PUT", {
      _id: emp._id,
      name: name,
    });
    if (!res.hasOwnProperty("status")) {
      alert(
        "Imagen de logo cambiada, actualice página para visualizar cambios"
      );
    } else {
    ...
    <>
      ...
      <CRow>
        <CCol xs="12" md="6">
          <CCard>
            <CForm
              onSubmit={handleSubmit(_actualizar)}
            ...
            >
              ...
              <CCardFooter>
                {errorMsg !== "" && (
                  <CFormText className="help-block">{errorMsg}</CFormText>
                )}
        ...
        <CCol xs="12" md="6">
          ...
          <CCard>
            <CForm onSubmit={handleSubmit2(_cambiarPwd)}>
              ...
              <CCardFooter>
                <CButton type="submit" size="sm" color="primary">
                  <CIcon name="cil-scrubber" /> Cambiar contraseña
                </CButton>
              </CCardFooter>
            </CForm>
          </CCard>
          <CCard color="danger" className="text-white">
            <CForm onSubmit={_eliminarCuenta} method="POST">
              ...
              <CCardBody>
                ¿Estás seguro de eliminar tu cuenta? Todos tus datos que han
                recopilado se elimarán. Para saber que esta decisión sea de
                manera consciente, ingrese su correo electrónico.
                <br />
                <CFormGroup>
                  <CInput
                    type="email"
              ...
              <CCardFooter color="danger" className="text-white">
                <CButton
                  ...
                  style={{ color: "black" }}
                >
                  <CIcon name="cil-scrubber" /> Borrar Cuenta
                </CButton>
              </CCardFooter>
            </CForm>
          </CCard>
